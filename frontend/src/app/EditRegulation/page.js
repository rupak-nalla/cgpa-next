...
export default function CreateRegulation() {
  ...
  const [semesters, setSemesters] = useState([{ semester: 1, subjects: [{ subjectName: '', credits: 0 }] }]);
  ...
  const addSemester = () => {
    setSemesters([...semesters, { semester: semesters.length + 1, subjects: [{ subjectName: '', credits: 0 }] }]);
  };
          ...
          <div className="container mx-auto  min-h-screen min-w-screen bg-slate-800 p-5">
               ...
               {semesters.map((semester, semesterIndex) => (
                    ...
                    {semester.subjects.map((subject, subjectIndex) => (
                         ...
                         <button type="button" onClick={() => addSubject(semesterIndex)} className="text-blue-500 px-4">
                         Add Subject
                         </button>
                    </div>
               ))}

               <button type="button" onClick={addSemester} className="text-blue-500 mb-4 ">
                    Add Semester
               </button>
